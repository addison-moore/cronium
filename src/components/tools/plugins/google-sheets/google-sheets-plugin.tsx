...
type GoogleSheetsCredentials = Omit<GoogleSheetsFormData, "name">;
...
function GoogleSheetsCredentialForm({
  ...
  const handleSubmit: SubmitHandler<GoogleSheetsFormData> = async (data) => {
...
export const GoogleSheetsPlugin: ToolPlugin = {
  ...
  async validate(
    ...
    const result = googleSheetsSchema.safeParse(credentials);
    if (result.success) {
