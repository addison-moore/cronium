...
export const createUserVariableSchema = z
  .object({
    key: z
      .string()
    ...
    value: z.string().max(10000, "Value must be less than 10,000 characters"),
    description: z
      .string()
  ...
  .refine(
    ...
    {
      message:
        "Variable key cannot start with reserved prefixes (CRONIUM_, SYSTEM_, NODE_, PATH, HOME)",
      path: ["key"],
    },
  );

// Update user variable schema
export const updateUserVariableSchema = z
  .object({
...
export const variableUsageSchema = z
  .object({
    variableId: z
      .number()
      .int()
      .positive("Variable ID must be a positive integer")
      .optional(),
...
export type BulkVariableOperationInput = z.infer<
  typeof bulkVariableOperationSchema
>;
