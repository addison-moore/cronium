...
export const createServerSchema = z.object({
  ...
  address: z.string().min(1, "Server address is required").max(255, "Address must be less than 255 characters")
    .refine((addr) => {
  ...
  username: z.string().min(1, "Username is required").max(50, "Username must be less than 50 characters").default("root"),
  port: z.number().int().min(1, "Port must be at least 1").max(65535, "Port must be less than 65536").default(22),
  description: z.string().max(500, "Description must be less than 500 characters").optional(),
  tags: z.array(z.string()).default([]),
  shared: z.boolean().default(false),
  maxConcurrentJobs: z.number().int().min(1, "Must allow at least 1 concurrent job").max(100, "Cannot exceed 100 concurrent jobs").default(5),
});
...
export const updateServerSchema = z.object({
  ...
  address: z.string().min(1).max(255)
    .refine((addr) => {
      ...
      const hostnameRegex = /^[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
      return ipv4Regex.test(addr) || hostnameRegex.test(addr);
    }, "Please enter a valid IP address or hostname").optional(),
  sshKey: z.string()
    .refine((key) => {
      // Allow empty string to keep existing key, or validate if provided
      if (!key || key.length === 0) return true;
      return key.includes("BEGIN") && key.includes("END") && key.includes("PRIVATE KEY");
    }, "SSH key must be in valid PEM format").optional(),
...
export const serverEventsSchema = z.object({
  ...
  limit: z.number().min(1).max(100).default(20),
  offset: z.number().min(0).default(0),
  status: z.enum(["ACTIVE", "DRAFT", "ARCHIVED", "ERROR", "RUNNING"]).optional(),
});

// Bulk server operations schema
export const bulkServerOperationSchema = z.object({
  serverIds: z.array(z.number().int().positive()).min(1, "At least one server must be selected"),
...
export type BulkServerOperationInput = z.infer<typeof bulkServerOperationSchema>;
export type TestServerConnectionInput = z.infer<typeof testServerConnectionSchema>;
