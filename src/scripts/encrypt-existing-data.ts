...
import { users, servers, envVars, apiTokens, systemSettings } from '../shared/schema';
 ...
 */
async function migrateUsers(): Promise<{ processed: number; encrypted: number }> {
  ...
  for (const user of allUsers) {
    ...
    if (user.password && !isDataEncrypted(user.password)) {
      try {
        ...
        encrypted++;
        console.log(`  ✓ Encrypted password for user: ${user.email || user.username || user.id}`);
      } catch (error) {
        console.error(`  ✗ Failed to encrypt password for user ${user.id}:`, error);
      }
    }
  }
 ...
 */
async function migrateServers(): Promise<{ processed: number; encrypted: number }> {
  ...
  for (const server of allServers) {
    ...
    if (server.sshKey && !isDataEncrypted(server.sshKey)) {
      try {
        ...
        console.error(`  ✗ Failed to encrypt SSH key for server ${server.id}:`, error);
      }
    }
  }
 ...
 */
async function migrateEnvVars(): Promise<{ processed: number; encrypted: number }> {
  ...
  for (const envVar of allEnvVars) {
    ...
    if (envVar.value && !isDataEncrypted(envVar.value)) {
      try {
        ...
        encrypted++;
        console.log(`  ✓ Encrypted environment variable: ${envVar.key} (Event ID: ${envVar.eventId})`);
      } catch (error) {
 ...
 */
async function migrateApiTokens(): Promise<{ processed: number; encrypted: number }> {
 ...
 */
async function migrateSettings(): Promise<{ processed: number; encrypted: number }> {
...
async function main() {
  ...
  try {
    ...
    console.log('\n📊 Migration Summary:');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log(`👥 Users:          ${stats.usersEncrypted}/${stats.usersProcessed} encrypted`);
    console.log(`🖥️  Servers:        ${stats.serversEncrypted}/${stats.serversProcessed} encrypted`);
    console.log(`🔧 Environment Vars: ${stats.envVarsEncrypted}/${stats.envVarsProcessed} encrypted`);
    console.log(`🔑 API Tokens:     ${stats.apiTokensEncrypted}/${stats.apiTokensProcessed} encrypted`);
    console.log(`⚙️  Settings:       ${stats.settingsEncrypted}/${stats.settingsProcessed} encrypted`);
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    
    const totalEncrypted = stats.usersEncrypted + stats.serversEncrypted + 
                          stats.envVarsEncrypted + stats.apiTokensEncrypted + 
                          stats.settingsEncrypted;
    const totalProcessed = stats.usersProcessed + stats.serversProcessed + 
                          stats.envVarsProcessed + stats.apiTokensProcessed + 
                          stats.settingsProcessed;
    
    console.log(`🎉 Total: ${totalEncrypted}/${totalProcessed} records encrypted`);
    
    if (totalEncrypted === 0) {
      console.log('\n✅ All sensitive data is already encrypted!');
    } else {
      console.log(`\n✅ Successfully encrypted ${totalEncrypted} records!`);
      console.log('⚠️  Remember to backup your ENCRYPTION_MASTER_KEY environment variable.');
    }
