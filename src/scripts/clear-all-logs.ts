...
async function clearAllLogs() {
  try {
    ...
    const countResult = await db.execute(sql`SELECT COUNT(*) FROM logs`);
    ...
    console.log(
      `Successfully deleted approximately ${logCount} logs from the database.`,
    );
    console.log(`Operation complete.`);

    return {
      success: true,
      message: `All logs have been deleted (approximately ${logCount} total).`,
    };
  } catch (error) {
...
async function clearEventLogs(eventId: number) {
  try {
    ...
    console.log(
      `Successfully deleted ${logCount} logs for event/script ID ${eventId}.`,
    );
    return {
      success: true,
      message: `Deleted ${logCount} logs for event/script ID ${eventId}.`,
    };
  } catch (error) {
...
async function clearOldLogs(daysAgo: number) {
  try {
    ...
    if (logCount === 0) {
      console.log(
        `No logs found older than ${daysAgo} days (${cutoffDate.toISOString()}).`,
      );
      return { success: true, message: "No logs to delete." };
    }
    ...
    console.log(
      `Successfully deleted ${logCount} logs older than ${daysAgo} days.`,
    );
    return {
      success: true,
      message: `Deleted ${logCount} logs older than ${daysAgo} days.`,
    };
  } catch (error) {
  ...
  try {
    ...
    if (args[0] === "--older-than") {
      ...
      if (isNaN(daysAgo) || daysAgo <= 0) {
        console.error(
          "Invalid days parameter. Please provide a positive number.",
        );
        process.exit(1);
      }
    ...
    console.log(
      "npx tsx scripts/clear-all-logs.ts                   - Clear all logs",
    );
    console.log(
      "npx tsx scripts/clear-all-logs.ts --event <id>      - Clear logs for specific event",
    );
    console.log(
      "npx tsx scripts/clear-all-logs.ts --older-than <days> - Clear logs older than specified days",
    );
    process.exit(1);
  } catch (error) {
