...
export const webhookQuerySchema = z.object({
  ...
  sortBy: z
    .enum(["name", "createdAt", "lastTriggered", "triggerCount"])
    .default("name"),
...
export const createWebhookSchema = z.object({
  ...
  key: z
    ...
    .regex(
      /^[a-zA-Z0-9_-]+$/,
      "Webhook key can only contain letters, numbers, underscores, and hyphens",
    )
...
export const webhookExecutionHistorySchema = z.object({
  ...
  offset: z.number().min(0).default(0),
  status: z
    .enum(["success", "failure", "timeout", "rate_limited", "unauthorized"])
    .optional(),
...
export const webhookStatsSchema = z.object({
  ...
  endDate: z.string().datetime().optional(),
  groupBy: z
    .enum(["status", "source_ip", "method", "hour", "day"])
    .default("status"),
});
...
export type WebhookExecutionHistoryInput = z.infer<
  typeof webhookExecutionHistorySchema
>;
...
export type WebhookPayloadValidationInput = z.infer<
  typeof webhookPayloadValidationSchema
>;
