...
func (sm *SidecarManager) CreateRuntimeSidecar(ctx context.Context, job *types.Job, networkID string) (string, error) {
	...
	containerConfig := &container.Config{
		...
		Env: []string{
			fmt.Sprintf("EXECUTION_ID=%s", job.ID),
			fmt.Sprintf("JWT_SECRET=%s", sm.executor.config.Runtime.JWTSecret),
			fmt.Sprintf("BACKEND_URL=%s", sm.executor.config.Runtime.BackendURL),
			fmt.Sprintf("BACKEND_TOKEN=%s", os.Getenv("CRONIUM_API_TOKEN")),
			fmt.Sprintf("VALKEY_URL=%s", sm.executor.config.Runtime.ValkeyURL),
			fmt.Sprintf("PORT=8081"),
			"LOG_LEVEL=info",
		},
